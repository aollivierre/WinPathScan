...
function Export-HTMLReport {
    ...
    New-HTML -Title "Long Paths Analysis Report" -FilePath $htmlPath -ShowHTML {
        New-HTMLTab -Name 'Summary' -Content {
...
"@
        ...
        New-HTMLTab -Name 'Details' -Content {
            New-HTMLSection -HeaderText "Long Paths Details" {
                New-HTMLTable -DataTable $Results -ScrollX -SearchBuilder -Buttons @('searchBuilder', 'csv', 'excel')
            }
        }
        
        if ($AccessErrors) {
            New-HTMLTab -Name 'Errors' -Content {
...
# Example usage:
Get-LongPathAnalysis -Path "C:\code" -Limit 25 -ExportFormat "Both" -IncludePermissions