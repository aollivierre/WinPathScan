...
function Get-LongPathItems {
    ...
    if (Test-Path $Path) {
        ...
        $totalFiles = ($results | Where-Object Type -eq 'File').Count
        $totalDirs = ($results | Where-Object Type -eq 'Directory').Count

        New-HTML -Title "Long Paths Report" -FilePath $ExportHtmlPath -ShowHTML {
            New-HTMLSection -HeaderText "Files and Directories Exceeding $Limit Characters" -Content {
                New-HTMLPanel {
                    New-HTMLText -Text "Total Files: $totalFiles<br>Total Directories: $totalDirs"
                }
                New-HTMLTable -DataTable $results -ScrollX -HideFooter
            }
        }

        Write-Host "Exported results to '$ExportHtmlPath'. Total items found: $($results.Count) (Files: $totalFiles, Directories: $totalDirs)"
    } else {
