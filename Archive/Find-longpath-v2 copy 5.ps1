...
function Get-LongPathFolders {
    ...
    if (Test-Path $Path) {
        ...
        # Group items by directory and count occurrences
        $results = $items | Group-Object DirectoryName | Select-Object Name, Count, @{Name="Files"; Expression={$_.Group | Sort-Object FullName.Length -Descending}} | Sort-Object Count -Descending
        ...
        New-HTML -Title "Folders with Long Paths Report" -FilePath $ExportHtmlPath -ShowHTML {
            New-HTMLSection -HeaderText "Folders with Files Exceeding $Limit Characters" -Content {
                New-HTMLTable -DataTable $results -Property Name, Count -ScrollX -HideFooter
            }
        }

        Write-Host "Report generated: '$ExportHtmlPath'. Total directories found: $($results.Count)"
    } else {
        Write-Warning "The path '$Path' does not exist."
    }
}
...
# Example call to the function:
# Make sure to replace "C:\YourTargetPathHere" with the actual path you want to analyze
Get-LongPathFolders -Path "C:\Users\user\UpHouse Inc"
